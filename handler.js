...
const antibadwordCmd = require('./commands/admin/antibadword');
const antistickerCmd = require('./commands/admin/antisticker');
    ...
    for (let participant of participants) {
      ...
      if (action === 'add' && groupSettings.welcome) {
        try {
          ...
          const welcomeMsg = `🎉✨ *নতুন মেম্বার স্বাগতম!* @${displayName}\n\n` +
                             `⚖️ *গ্রুপ:* *${groupName}*\n\n` +
                             `🚀 *তুমি এখন আমাদের গ্রুপের অংশ!*\n` +
                             `👥 *মোট মেম্বার:* *${groupMetadata.participants.length}*\n` +
                             `📢 *নিয়ম:* *সম্মান দেখাও & অ্যাক্টিভ থাকো*\n\n` +
                             ` ╰┈➤ *পাওয়ার্ড বাই বট*`;
      ...
      // --- GOODBYE/LEFT LOGIC ---
        ...
        const finalMessage = `@${displayName} *_কিরে দুই টাকার ফকিন্নির পোলা তোর গ্রুপে থাকার যোগ্যতা নাই যা ভাগ_*`;
    ...
    if (linkPattern.test(body)) {
      ...
      if (action === 'kick' && botIsAdmin) {
        try {
          ...
          await sock.groupParticipantsUpdate(from, [sender], 'remove');
          await sock.sendMessage(from, { 
            text: `*🔗 কিরে ফকিন্নির পোলা আব্বুর গ্রুপে লিংক মারস তোর লিংক তোর ওদিক দিয়া ঢুকায়া দিমু*`,
            mentions: [sender]
          }, { quoted: msg });
        } catch (e) {
